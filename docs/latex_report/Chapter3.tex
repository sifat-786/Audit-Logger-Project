...
% -------------------------------------------------------
% FIGURE 1: High-Level System Architecture Flowchart
% -------------------------------------------------------
\begin{figure}[H]
\centering
\begin{tikzpicture}[
  ...
  box/.style = {rectangle, rounded corners=4pt, draw, thick, minimum width=3.0cm, minimum height=0.8cm, align=center, font=\scriptsize},
  arrow/.style = {-{Stealth[scale=1.0]}, thick},
  layer/.style = {rectangle, rounded corners=6pt, draw, dashed, thick, inner sep=8pt}
]
...
% Controller
\node[box, fill=lightyellow!20, draw=warning, below=1.4cm of ui]    (ctrl) {\textbf{Controlled Intent Router}\\\scriptsize agent/controller.py};

% Governance row
\node[box, fill=lightorange!15, draw=warning, below left=1.4cm and 1.8cm of ctrl]  (risk) {\textbf{Risk Classifier}\\\scriptsize risk\_classifier.py};
\node[box, fill=lightorange!15, draw=warning, below=1.4cm of ctrl]                  (dec)  {\textbf{Decision Engine}\\\scriptsize decision\_engine.py};
\node[box, fill=lightorange!15, draw=warning, below right=1.4cm and 1.8cm of ctrl] (exp)  {\textbf{Explanation Engine}\\\scriptsize explanation\_engine.py};

% Tools vs Block
\node[box, fill=lightgreen!15, draw=success, below left=1.4cm and 0.8cm of dec]    (tools) {\textbf{Tool Executor}\\\scriptsize gmail / generic / scheduler};
\node[box, fill=lightred!15, draw=danger, below right=1.4cm and 0.8cm of dec]      (block) {\textbf{Blocked / Halted}\\\scriptsize Decision: reject / hold};

% Audit Logger
\node[box, fill=lightpurple!15, draw=critical, below=1.4cm of tools]               (audit) {\textbf{Audit Logger}\\\scriptsize agent/audit\_logger.py};

% Storage backends
\node[box, fill=codebg, draw=muted, below left=0.9cm and 1.0cm of audit]        (json)  {\textbf{JSON}\\\scriptsize audit\_log.json};
\node[box, fill=codebg, draw=muted, below=0.9cm of audit]                        (csv)   {\textbf{CSV}\\\scriptsize audit\_log.csv};
\node[box, fill=codebg, draw=muted, below right=0.9cm and 1.0cm of audit]       (db)    {\textbf{SQLite}\\\scriptsize audit\_log.db};
...
\node[left=1.5cm of ui, font=\scriptsize\bfseries\color{accent}, align=right] {Layer 1:\\Presentation};
\node[left=1.5cm of ctrl, font=\scriptsize\bfseries\color{warning}, align=right] {Layer 2:\\Orchestration};
\node[left=1.5cm of dec, font=\scriptsize\bfseries\color{warning}, align=right] {Layer 3:\\Governance};
\node[left=1.5cm of audit, font=\scriptsize\bfseries\color{critical}, align=right] {Layer 4:\\Telemetry};
...
% -------------------------------------------------------
% FIGURE 2: Request Lifecycle Flowchart (Optimized to prevent node overlap)
% -------------------------------------------------------
\begin{figure}[H]
\centering
\begin{tikzpicture}[
  ...
  startstop/.style={rectangle, rounded corners=10pt, draw, thick, minimum width=2.4cm, minimum height=0.6cm, align=center, font=\scriptsize\bfseries},
  process/.style={rectangle, draw, thick, minimum width=2.6cm, minimum height=0.65cm, align=center, font=\scriptsize},
  decision/.style={diamond, draw, thick, minimum width=2.8cm, minimum height=0.9cm, align=center, font=\scriptsize, aspect=2.2},
  arrow/.style={-{Stealth[scale=1.0]}, thick}
]
...
\node[process, fill=lightgreen!15, draw=success, right=1.5cm of tool?] (chat)   {Normal Chat\\ Response};
...
\node[process, fill=lightgreen!15, draw=success, right=1.5cm of tier]  (allow)  {Auto-Approve \&\\ Execute Tool};
\node[process, fill=lightorange!15, draw=warning, below=of tier]       (hold)   {Halt Execution\\ Show Approval Card};
\node[process, fill=lightred!15, draw=danger, left=1.5cm of tier]     (block)  {Block Action\\ CRITICAL Reject};

\node[decision, fill=white, draw=warning, below=of hold]           (approved?) {User\\ Approved?};
\node[process, fill=lightgreen!15, draw=success, right=1.5cm of approved?] (exec)  {Execute Tool\\ \& Synthesize};
\node[process, fill=lightred!15, draw=danger, left=1.5cm of approved?] (skip)  {Remain\\ Blocked};

% Write Audit Log is positioned below approved? to prevent overlapping with hold/approved?
\node[process, fill=lightpurple!15, draw=critical, below=1.0cm of approved?] (log)    {Write Audit Log\\\scriptsize(audit\_logger.py)};
...
\draw[arrow, color=critical] (allow)  |- (log);
\draw[arrow, color=critical] (exec)   -- (log);
\draw[arrow, color=critical] (block)  |- (log);
...
% -------------------------------------------------------
% FIGURE 3: Module Dependency Graph (Sized to prevent overlap)
% -------------------------------------------------------
\begin{figure}[H]
\centering
\begin{tikzpicture}[
  ...
  mod/.style={rectangle, rounded corners=3pt, draw, thick, minimum width=2.4cm, minimum height=0.6cm, align=center, font=\scriptsize},
